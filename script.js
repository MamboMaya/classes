...
class Factory {
  constructor(){
    ...
    this.massBuild = function(quantity, options){
      ...
      this.options = `${this.color} ${this.trim} ${this.model}`
      return `Building ${this.quantity} ${this.options}'s`
    }
    this.customerBuild = function(color, options){
      this.color = color
      this.options = [options]
      return `Building one ${this.color} ${this.model} ${this.trim} with the following options: ${this.options}`
    }
  }
}
...
// var mazda3 = new Car('mazda3', 4, 'red', 'hybrid', 'automatic', 'touring', 'base', 'premium', 'leather', true)
...
const miataRf = new Sport('Miata-RF', 'Grand Touring',
'manual', 'hard top', 'Red', 'leather', 'premium', 'premium')
